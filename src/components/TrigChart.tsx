...
const zeroLinePlugin = {
  ...
  afterDraw(chart: ChartJS, _args: unknown, opts: { color?: string }) {
    ...
    for (let v = Math.ceil(x.min / step) * step; v <= x.max; v += step) {
...
const markerPlugin = {
  ...
  afterDraw(chart: ChartJS, _args: unknown, opts: { angle?: number; fnY?: number | null; derivY?: number | null; color?: string; ping?: number }) {
    ...
    function drawSkier(py: number, alpha: string, r: number, _glow: number) {
      ctx.save();
      // Ping ring
      if (ping) {
        const pr = 4 + ping * 20;
    ...
    if (fnY !== null && fnY !== undefined && isFinite(fnY)) drawSkier(yScale.getPixelForValue(fnY), '', 7, 10);
    if (derivY !== null && derivY !== undefined && isFinite(derivY)) drawSkier(yScale.getPixelForValue(derivY), '88', 7, 6);
  },
};
